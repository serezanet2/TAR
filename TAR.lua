...
local function shouldSkipItem(prompt)
    ...
    if lowerName:find("blood") or lowerName:find("garlic") or lowerName:find("oil") then return true end
...
-- Кнопка сворачивания (прячет панель и показывает круглую кнопку TARC)
...
local function showMainPanel()
    tarcButton.Visible = false
    frame.Visible = true
    frame.Position = UDim2.new(-0.5, -100, 0.5, -80) -- исходная позиция для твина
